...
/// Used by [DropdowFeild.selectedItemBuilder].
...
// DropdowFeild menus should never overscroll or display an overscroll indicator.
  ...
  Widget build(BuildContext context) {
    ...
    return FadeTransition(
      ...
      child: CustomPaint(
          ...
          //   // elevation: route.elevation.toDouble(),
          //   borderColor: ml.Colors.grey, // theme.resources.surfaceStrokeColorFlyout,
          //   backgroundColor: widget.popupColor,
          // ),
          child: ClipRRect(
...
class _ComboBoxRoute<T> extends PopupRoute<_ComboBoxRouteResult<T>> {
  ...
  final List<DropdownFeildItem<T>> items;
...
class _ComboBoxRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // DropdowFeild.itemHeight is specified or DropdowFeild.itemHeight is null
...
// The container widget for a menu item created by a [DropdowFeild]. It
// provides the default configuration for [DropdownFeildItem]s, as well as a
// [DropdowFeild]'s placeholder and disabledPlaceholder widgets.
...
/// An item in a menu created by a [DropdowFeild].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class DropdownFeildItem<T> extends _ComboBoxItemContainer {
  /// Creates an item for a combo box menu.
  ///
  /// The [child] argument is required.
  const DropdownFeildItem({
    Key? key,
    this.onTap,
    this.value,
    required Widget child,
  ...
  ///
  /// Eventually returned in a call to [DropdowFeild.onChanged].
  final T? value;
}
...
///
/// ![DropdowFeild Popup preview](https://docs.microsoft.com/en-us/windows/apps/design/controls/images/combo-box-list-item-state.png)
///
/// The type `T` is the type of the [value] that each combo box item represents.
/// All the entries in a given menu must represent values with consistent types.
/// Typically, an enum is used. Each [DropdownFeildItem] in [items] must be
...
/// See also:
///
///  * [DropdownFeildItem], the class used to represent the [items].
///  * <https://docs.microsoft.com/en-us/windows/apps/design/controls/combo-box>
class DropdowFeild<T> extends StatefulWidget {
  /// Creates a combo box button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [DropdownFeildItem] values. If [items] or
  ...
  final List<DropdownFeildItem<T>>? items;
  final ml.InputBorder? border;
  final ml.InputDecoration? decoration;
  final bool? hideSuffix;

  /// The value of the currently selected [DropdownFeildItem].
  ///
  /// If [value] is null and the button is enabled, [placeholder] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [DropdowFeild.items]
  /// is null then the combo box button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [DropdowFeild.disabledPlaceholder] widget if it is non-null.
  /// If [DropdowFeild.disabledPlaceholder] is also null but [DropdowFeild.placeholder] is
  /// non-null, [DropdowFeild.placeholder] will instead be displayed.
  ...
  /// [DropdownFeildItem]s in [items].
  ///
  /// When a [DropdownFeildItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropdownFeildItem] of the same index
  /// in [items].
  ///
  /// This sample shows a `DropdowFeild` with a button with [Text] that
  /// corresponds to but is unique from [DropdownFeildItem].
  ///
  /// ```dart
  ...
  ///         return DropdownFeildItem<String>(
  ///           child: Text('Log $item'),
  ///           value: item,
  ///         );
  ///       }).toList(),
  ...
  /// ```
  ///
  /// If this callback is null, the [DropdownFeildItem] from [items]
  ...
  ///
  /// This sample shows a `DropdowFeild` with a combo box button text style
  ...
  ///         }).toList();
  ///       },
  ///       items: options.map<DropdownFeildItem<String>>((String value) {
  ///         return DropdownFeildItem<String>(
  ///           value: value,
  ///           child: Text(value),
  ///         );
  ...
  @override
  State<DropdowFeild<T>> createState() => ComboBoxState<T>();
}

class ComboBoxState<T> extends State<DropdowFeild<T>> {
  int? _selectedIndex;
  ...
  @override
  void didUpdateWidget(DropdowFeild<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    if (widget.items!.any(
        (DropdownFeildItem<T> item) => item.value == widget.controller!.text)) {
      for (var itemIndex = 0; itemIndex < widget.items!.length; itemIndex++) {
        if (widget.items![itemIndex].value == widget.controller!.text) {
          _selectedIndex = itemIndex;
          widget.controller!.text = widget.items![itemIndex].value.toString();
          return;
        }
      }
    }
  ...
  TextStyle? get textStyle => widget.style ?? fluentThemeData!.typography.body;
  ...
  void openPopupOld() {
    ...
    assert(isEnabled, 'The DropdowFeild must be enabled to open a popup');
    final textDirection = Directionality.maybeOf(context);
    const EdgeInsetsGeometry menuMargin = _kAlignedMenuMargin;
  ...
  void onChanged(T? newValue) {
    ...
    _selectedIndex = widget.items!
        .indexWhere((element) => element.value == newValue.toString());
    widget.controller!.text = newValue.toString();
    if (widget.onChanged != null) widget.onChanged!(newValue);
  }

  Color iconColor(BuildContext context) {
    final res = fluentThemeData!.resources;
    if (isEnabled) {
        ...
        child: ml.Focus(
          ...
          child: InfoLabel(
              ...
              child: widget.isFluent
                  ? TextFormBox(
                      readOnly: true,
                      onTap: isEnabled ? openPopup : null,
                      focusNode: focusNode,
                      validator: widget.validator,
                      controller: widget.controller,
                      style: widget.style,
                      textInputAction: TextInputAction.next,
                      suffix: const Icon(ml.Icons.arrow_drop_down,
                          color: ml.Colors.black54),
                    )
                  : ml.TextFormField(
                      readOnly: true,
                      onTap: isEnabled ? openPopup : null,
                      ...
                      decoration: widget.decoration ??
                          ml.InputDecoration(
                            errorStyle: widget.headerStyle,
                            suffixIcon: widget.hideSuffix!
                                ? null
                                : const Icon(
                                    ml.Icons.arrow_drop_down,
                                    color: ml.Colors.black54,
                                  ),
                            isDense: true,
                            ...
                            filled: true,
                            fillColor: ml.Colors.white70,
                            constraints: widget.constraints,
                            border: widget.border ??
                                ml.OutlineInputBorder(
