...
class ModelRep extends StatefulWidget {
  // declare brand id, start date, end date
  final String? brandName;
  final DateTime? start, end;
  // contructor to accept this values
  const ModelRep({this.brandName, this.start, this.end, super.key});

  @override
  State<ModelRep> createState() => _ModelRepState();
}

class _ModelRepState extends State<ModelRep> {
  List<int> xdata = [];
  ...
  int durationIndex = 6;
  List<ModelReport> modelreportData = [];
  List<ModelReport> tempData = [];
  List<String> modelgroupData = [
    'Category wise',
    'Subcategory wise',
    'Detail wise'
  ];
  ...
  Future<void> exportExcelReport() async {
    ...
    int rowIndex = 0;
    List<List<String>> detailsData = modelreportData.map((e) {
      rowIndex++;
      return [
  ...
  void pagination() {
    if ((scrollController.position.pixels >=
        ...
        (pageIndex <= modelreportData.length) &&
  ...
  loadData() async {
    await mysql_class
        .loadmodelReport(start!, end!, widget.brandName)
        .then((value) {
      loading.value = false;
      if (mounted) {
        modelreportData = value;
        tempData = modelreportData;
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: searchActive
              ...
              : Text(
                  'Model Report (${modelreportData.length})',
                  style: GoogleFonts.poppins(
                      fontSize: 17,
                      fontWeight: FontWeight.w500,
          ...
          child: Column(
            ...
            children: [
              ListView.separated(
                  ...
                  itemCount: pageIndex < modelreportData.length
                      ? pageIndex
                      : modelreportData.length,
                  itemBuilder: (context, index) {
                    return Padding(
                      ...
                      child: Material(
                        ...
                        child: ListTile(
                          onTap: () {
                            // open transaction report of same model, model, date range
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => ReportsSreen(
                                        start: start,
                                        end: end,
                                        modelID:
                                            modelreportData[index].modelID)));
                          },
                              ...
                              children: [
                                Row(
                                    ...
                                    children: [
                                      Expanded(
                                        child: Text(
                                            "${modelreportData[index].modelname}",
                                            style: GoogleFonts.poppins(
                                                fontWeight: FontWeight.w500,
                                                fontSize: 15)),
                                      ),
                                      ...
                                      Expanded(
                                        child: Text(
                                            "${mysql_class.simpleCurreny(double.parse(modelreportData[index].amt!))}",
